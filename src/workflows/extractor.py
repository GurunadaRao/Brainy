...
from src.domain.models import Chunk, Transcription, Triplet
...
async def process_video_extraction(
    video_id: str, confidence_threshold: float = 0.7
) -> Dict[str, Any]:
    """
    ...
    for chunk_content in chunks_text:
        ...
        filtered_triplets = [
            t for t in triplets if t.get("confidence", 1.0) >= confidence_threshold
        ]
        print(
            f"Extractor: Extracted {len(filtered_triplets)} high-confidence triplets."
        )
    ...
    print(
        "Extractor: Streaming extractions to Graph (Neo4j) and Vector (Qdrant) databases..."
    )
    ...
    for payload in graph_load_payload:
        ...
        vector_client.upsert_chunk(
            ...
            payload={"content": payload["content"], "video_id": video_id},
        )

    print(
        f"Extractor: Knowledge extraction and graph construction complete for video {video_id}."
    )
    return {
        "video_id": video_id,
